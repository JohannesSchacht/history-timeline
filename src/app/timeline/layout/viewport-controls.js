...
export function clampViewport(viewport) {
    ...
    if (start < ZOOM.minYear)
        start = ZOOM.minYear;
    if (start + span > ZOOM.maxYear)
        start = ZOOM.maxYear - span;
 ...
 */
export function zoomViewport(viewport, focusYear, factor) {
    const { startYear, endYear, widthPx } = viewport;
...
export function panViewport(viewport, dx) {
    const { startYear, endYear, widthPx } = viewport;
 ...
 */
export function wheelZoomFactor(deltaY, boost = 1) {
 ...
 */
export function wheelBoost(streak) {
