import { yearToX } from './time-scale';
...
export function tickStep(spanYears) {
...
export function buildTicks(viewport) {
 ...
 */
function packLanes(intervals, gap) {
    const laneEnds = [];
...
export function layoutTimeline(events, viewport) {
    ...
    const points = pointEvents.map(({ event }, i) => ({
        event,
        x: pointXs[i],
    ...
    const spans = spanEvents.map(({ event }, i) => ({
        event,
        x1: spanRects[i].x1,
        x2: spanRects[i].x2,
